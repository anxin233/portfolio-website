-- ============================================
-- Portfolio 个人网站数据库初始化脚本
-- 使用方式：在 MySQL 客户端中执行此文件
...
CREATE TABLE IF NOT EXISTS `blog_post` (
  ...
  `content`    TEXT                                  COMMENT '文章内容（Markdown）',
  `category`   VARCHAR(50)  NOT NULL                COMMENT '分类：前端/后端/架构/团队',
  `tags`       VARCHAR(255) DEFAULT NULL             COMMENT '标签，逗号分隔',
  `read_time`  INT          DEFAULT NULL             COMMENT '预计阅读时间（分钟）',
  `published`  TINYINT(1)   NOT NULL DEFAULT 0       COMMENT '是否发布：0=草稿 1=已发布',
